...
class IntIntMap : MutableMap<Int, Int> {
     ...
     */
    constructor(map: IntIntMap) : this((map.keyTable.size * map.loadFactor).toInt(), map.loadFactor) {
    ...
    override val entries: MutableSet<MutableMap.MutableEntry<Int, Int>>
        get() = entries() as MutableSet<MutableMap.MutableEntry<Int, Int>>
    ...
    override fun containsValue(value: Int): Boolean {
        val valueTable = valueTable
        if (value == 0) {
            if (hasZeroValue && zeroValue == 0) return true
            val keyTable = keyTable
            for (i in valueTable.indices.reversed()) if (keyTable[i] != 0 && valueTable[i] == 0) return true
        }
        else {
    ...
    fun findKey(value: Any?): Int? {
        val valueTable = valueTable
        if (value == null) {
            if (hasZeroValue && zeroValue == 0) return 0
            val keyTable = keyTable
            for (i in valueTable.indices.reversed()) if (keyTable[i] != 0 && valueTable[i] == 0) return keyTable[i]
        }
        else {
    ...
    override fun hashCode(): Int {
        var h = size_
        if (hasZeroValue && zeroValue != 0) h += zeroValue.hashCode()
        val keyTable = keyTable
        val valueTable = valueTable
        var i = 0
        val n = keyTable.size
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (hasZeroValue) {
            if (other.zeroValue == 0) {
                if (zeroValue != 0) return false
            }
            else {
    ...
    class Values(map: IntIntMap) : MutableCollection<Int>, MapIterator<Int, Int>(map) {
        override fun hasNext(): Boolean {
            if (!valid) throw RuntimeException("#iterator() cannot be used nested.")
            return hasNext
        }
    ...
    class Keys(map: IntIntMap) : MutableSet<Int>, MapIterator<Int, Int>(map) {
        override fun hasNext(): Boolean {
            if (!valid) throw RuntimeException("#iterator() cannot be used nested.")
            return hasNext
        }
        ...
        override fun addAll(elements: Collection<Int>): Boolean {
            ...
            elements.forEach {
                alreadyAdded = alreadyAdded || map.put(it, 0) == null
            }
